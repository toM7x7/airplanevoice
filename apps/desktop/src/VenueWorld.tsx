...
function Label({
  ...
  const texture = useMemo(() => {
    ...
    ctx.fillText(text, canvas.width / 2, 52, canvas.width - 12);
  ...
  return (
    <sprite
      position={position}
      scale={compact ? [0.12, 0.075, 1] : [0.75, 0.12, 1]}
    >
...
export function VenueWorld({
  ...
  const h = venue.tableHeightM,
    d = venue.baselineM,
    depth = tableDepth(venue);
  return (
    <group>
      {markers && (
        <group>
          <mesh
            position={[d / 2, h, -depth / 2]}
            rotation={[-Math.PI / 2, 0, 0]}
          >
            <planeGeometry args={[d, depth]} />
            <meshBasicMaterial
              color="#d8eee5"
              wireframe
