...
export class SkyRoom extends DurableObject<Env> {
  ...
  async initialize(keyhash: string) {
  ...
  async fetch(request: Request) {
    ...
    server.serializeAttachment({ window: Date.now(), count: 0 });
  ...
  webSocketMessage(socket: WebSocket, message: string | ArrayBuffer) {
    ...
    try {
      ...
      const next = changeRoom(state, op, Date.now());
      if (next !== state)
        this.ctx.storage.sql.exec(
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/api/rooms" && request.method === "POST") {
        ...
        await env.ROOMS.getByName(id).initialize(await digest(key));
