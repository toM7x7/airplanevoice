...
const out = process.env.TRAFFIC_OUTPUT || "output/radar-2026-09-23";
...
try {
  ...
  checks.push("local server persists all six capacity choices; Jev stays off");
  ...
  const fixture = await p.evaluate(async (root) => {
    ...
    const now = Date.now(),
      s = newRoom(now, true);
    s.traffic = { capacity: 24, jev: false };
    s.persistent = true;
  ...
  await q.getByRole("button",{name:"フライトレーダー",exact:true}).click();
  await q.getByRole("button",{name:"音響マップ",exact:true}).focus(); await q.keyboard.press("Enter");await q.waitForTimeout(800);
  await q.locator(".flight-radar").screenshot({path:out+"/pc-sound-map.png"});
  await q.getByRole("button",{name:"レーダーを閉じる"}).focus(); await q.keyboard.press("Enter");
  await q.locator("#vr-enter").click();
  await q.waitForFunction(
    () => JSON.parse(window.render_game_to_text()).vr.frames > 20,
  );
  const press = async (label, radar=false) => {
    if (!radar && label !== "メニューを開く" && !(await state(q)).vr.controls.open)
      await press("メニューを開く");
    if (!radar && label !== "メニューを開く")
      await q.waitForFunction(
        () =>
          JSON.parse(window.render_game_to_text()).vr.controls.progress === 1,
      );
    const vr = (await state(q)).vr, panel = radar ? vr.radar : vr.panel, origin=vr.origin, controls=radar ? vr.radar : vr.controls,
      t = controls.targets.find((t) => t.label === label);
    assert(t && t.enabled !== false, label);
  ...
  await press("フライトレーダー");
  await q.waitForFunction(()=>JSON.parse(window.render_game_to_text()).vr.radar?.visible);
  await press("音響マップ",true);
  await q.waitForFunction(()=>JSON.parse(window.render_game_to_text()).vr.radar.mode==="sound");
  await q.screenshot({path:out+"/xr-sound-map.png"});
  await press("機体・航路",true);
  await press("テスト旅客機 24",true);
  assert((await state(q)).inspection?.id);
  const firstPick=(await state(q)).inspection.id;
  await press("テスト旅客機 24",true);
  assert.notEqual((await state(q)).inspection?.id,firstPick,"overlapping aircraft cycle");
  await press("閉じる",true);assert(!(await state(q)).vr.radar.visible);
  checks.push("independent XR radar toggles sound/flight and cycles overlapping aircraft; PC acoustic map displays");
  await press("飛行中の機体");
  const seen = [];
  for (let i = 0; i < 6; i++) {
    ...
    if (i < 5) await press("次の4機");
  }
  assert.equal(new Set(seen).size, 24);
  const last = (await state(q)).vr.controls.targets.find((t) =>
    t.label.includes("テスト旅客機 24"),
  );
  await press(last.label);
  assert.equal((await state(q)).inspection?.id, "ST-24");
  await press("空のメニューに戻る");
  await press("音・見え方の設定");
  await press("音量・聴き方");
  await press("聴く機体を選ぶ");
  await press("選んだ機体だけ聴く");
  await q.waitForFunction(()=>JSON.parse(window.render_game_to_text()).audio.mixGains["ST-24"]===1);
  assert(Object.entries((await state(q)).audio.mixGains).filter(([id])=>id!=="ST-24").every(([,gain])=>gain===0));
  await press("空全体を聴く");
  await q.waitForFunction(()=>Object.values(JSON.parse(window.render_game_to_text()).audio.mixGains).filter(g=>g>0).length===24);
  checks.push("XR solo selection reaches actual audio buses; balanced restores all 24");
  ...
  await press("機体を編集");
  await press("寸法");
  const oldBody=(await state(q)).creation.entry.recipe.aircraft.bodyLengthM;
  await press("胴体 ＋2m");
  assert.equal((await state(q)).creation.entry.recipe.aircraft.bodyLengthM,oldBody+2);
  assert.equal((await state(q)).fleet.filter(f=>f.started&&!f.ended).length,24);
  await q.screenshot({path:out+"/xr-edit-while-flying.png"});
  checks.push("XR model dimensions change while 24 shared aircraft keep flying");
  const info = await q.evaluate(() => window.__soundTrailRender);
  assert.deepEqual(errors, []);
  checks.push(
    "twenty-four named aircraft render in stereo, paginate across six pages, and aircraft 24 can be selected",
  );
  await fs.writeFile(
    out + "/result.json",
