...
const out=process.env.PUBLIC_OUTPUT||'output/traffic-vr-2026-09-23/public',base='https://airplanevoice.pages.dev';
...
try{
 ...
 const s=await get();assert.equal(s.room.state.traffic.capacity,6);assert.equal(s.room.state.traffic.jev,false);
 await p.getByRole('button',{name:'部屋・運営',exact:true}).click();
 assert.equal(await p.getByLabel('同時に飛ばす機体数').inputValue(),'6');
 ...
 const xr=(await get()).vr;assert.equal(xr.views,2);assert(xr.controls.targets.some(t=>t.label==='ARで見る'));assert(xr.controls.targets.some(t=>t.label==='手元へ呼ぶ'));
 await p.screenshot({path:out+'/xr.png'});
 assert(!calls.some(p=>/\/(live|ask|observe)$/.test(p)));assert.deepEqual(errors,[]);
 await fs.writeFile(out+'/result.json',JSON.stringify({checks:['Pages HTML, JS and offline bundle match local build','persistent exhibition migrated to six aircraft with Jev off','operator controls available','new VR dock renders in stereo','no paid client AI requests or page errors'],entry,errors},null,2));
