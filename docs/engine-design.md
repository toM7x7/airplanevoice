...
更新: 2026-09-17 / v0.11.0。単体エンジンの説明はv0.7.0を基礎に、共有・会場の境界を追記。全体の優先は[再設計書](experience-redesign.md)。

2026-09-18追加: 「何を選ぶか」の内部処理を、ルール／TypeSafeで差し替える[実験候補](typesafe-ai.md)を整理。次周の飛び方、音の性格、部品構成、管制判断を対象にできる。提案専用に固定せず、許可した範囲の自動適用も比較する。まだ接続・実装はしていない。下記は現在動いているコードの責務であり、AIを置ける場所を限定する表ではない。

`VenueMap`は部屋共通のm座標、`VrRuntime`の校正は端末の位置・回転、`venue-overview.ts`は表示専用の等方縮尺。`VenueOverview`の配置・倍率は耳・飛行・ルーム状態へ渡さない。会場画像、案内飛行、LLM接続は未実装。

2026-09-16追記：共有版v0.9.1でQR生成、VRメニュー、状況の事実、発話のライフサイクルを分離。[責務の見直しと次の境界](tower-and-voice.md#今回の見直しと次の境界)を参照。以下の単体エンジン構成に、LLMの待機処理は追加していない。
...
傾きに使う曲率は、768点の元曲線で計算し、等距離512点へ位置と一緒に補間する。等距離に並べ直した位置から曲率を再計算すると、小さな補間誤差が翼の細かな傾きとして出るため、v0.6.1で修正した。位置・接線・周回時間は維持し、チェックサムには姿勢計算の版も含める。

その曲率に対して、実験室では0〜3秒の有限な過去を8点で重み付き平均し、傾きの追従を調整する。プリセットの追従時間は0秒。元曲線の修正は追従時間0でも有効であり、履歴依存の描画補正を加えず、同じ時刻から同じ姿勢を再現する。質量・揚力・慣性モーメントから運動を解く段階には達していない。
...
最初の航路を1組だけ保持し、編集復帰で戻す。周回の保存を明示した場合は現在の航路を残す。Undo・ログ・航跡・音声は既存の上限に従う。v0.6.0から自動継続設定も端末保存するが、再読込後は編集状態で止まり、自動発進しない。`FlightMap`はコンパイル済みの現在の主航路を描き、飛行中のST-01の位置を表示する。音の分布図や実測マップではない。
...
v0.7.0の演目では`compileShow`が全機の個別レシピを検証し、`buildAirspace`へ独立した航路と開始時刻を渡す。全機の終了時刻の最大値を演目の長さとし、到来キューが空になるまで次の試演へ進まない。詳細は[演目設計](show-workshop.md)。下記の位置ずらしは通常の航路モードに適用する。

- 追加機の水平移動: ST-02 `(480, -420)` m、ST-03 `(-560, -780)` m（X, Z）。高さはそれぞれ最大+85m、+165m、650mを超えないように抑える。
- 通常モードの開始間隔は0、8、16秒。元航路の形状、長さ、速度は共有。演目モードでは各機の2地点航路・速度・0〜180秒の開始を独立させる。各機別の自由描線は今後。
...
合成音は`engine-sound.ts`で双発／四発の2つの4秒バッファを生成する。低域・中域・薄い空気音とファン成分を重ね、素材のRMSを0.155へ合わせる。出力先の補正中心は480Hz。既存の機体別バス、遅延・ドップラー、最大18音声を維持し、到来時に対象機のエンジン数で素材を選ぶ。実録音・機種別の物理音響モデルではない。
...
この単体の管制イベント表示は字幕のみ。共有版は`observeRoom`／定型文／`RoomSpeech`による端末読み上げを実装済み。外部AI・音声APIは未接続。

## 複数Questの共有境界（実装済み・実機受入は一部継続）

v0.8以降で予定と共通時計を共有し、v0.10で手動の位置合わせを追加済み。以下はその設計根拠。既存のQR入室・同期はユーザー実機確認済み、Quest 3＋2の机との一致は[別の実機受入](table-alignment.md)として残す。

2026-09-16に、初回は**PC＋Questで両方から編集・飛行開始**と確認。[共有ルームの具体案](shared-sky.md)へ、クラウドと端末の分担、下書き／飛行／次便の分離、競合・途中参加・個人休憩、初回の5段階を整理した。ドーム・LLMは着手の前提にしない。飛行の成立と機体間の間隔は各エンジンの責務とし、現行に衝突回避があるとは扱わない。
