...
最新更新：VR主要ページの操作整理、標準6機／最大12機、裏方Jevの出発間隔調整を公開。Worker `10411b47-6b7d-4581-bd28-dee777cee2d1`、Pages `0c74d736`、bundle `f6409a62019274ba`。[変更・確認結果](vr-traffic-2026-09-23.md)。公開配信・共有設定・模擬VR確認済み。以下の公開IDは前回までの履歴。

現在は [共通展示入口](https://airplanevoice.pages.dev/?shared=1) から固定のクラウド空間へ自動接続する。クラウド格納庫をPC/Questで共有し、8時間で終了しない。公開Worker `38b4e000-8aaa-45bf-a3d1-ae4e19cc3dc2`、Pages `ee73608b`。[構成・操作・検証](cloud-exhibition-2026-09-23.md)。以下のv0.10.0以前の期限・AI未接続という記述は当時の履歴。個別招待部屋の1時間/8時間期限は現在も有効。
...
今回のARは同じARセッション内の背景切り替え。机の手動座標合わせと地点登録はv0.10.0で追加。会場図の画像・ブース間の案内飛行は後続。QRは部屋の8時間の期限内で再利用でき、翌日以降も使う恒久ロビーではない。Quest実機の透過映像・文字・音と装着し直した際の復帰を確認する。
