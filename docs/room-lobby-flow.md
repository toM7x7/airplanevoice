...
更新: 2026-09-16 / v0.9.0で展示入口の初回実装。公開・実機確認の結果は進捗に記録。
...
以下はv0.8.1のSharedApp、room-client、room-worker、shared-roomを確認した設計時の比較。v0.9.0の追加は冒頭を参照。
...
| 招待・権限 | 招待URLを持つ人は編集できる | 公開入室と、作成者の管理権限を分ける |
| 参加・復帰 | 招待URLで同じ部屋へ再接続できる | 一覧からも入室でき、終了・満員を案内する |
| 有効期間 | 作成から1時間。終了時に内容削除 | 展示中の終了・復帰を設計。来場者に新しいURLへの移動を求めない |
...
| 参加方法 | 初回候補は「共同制作・参加者が編集できます」。後続で「観察」を検討 |
